...
//				btnListar.setForeground(Constantes.skyblue);
			...
			public void mouseClicked(MouseEvent e) {
				if(btnBuscar.isEnabled()){
					...
					if(getBusquedaCodigo().contentEquals("") || cboSelect == 0){
						...
						if(cboSelect == 2){
							if(term.matches("[a-zA-ZñÑáéíóúÁÉÍÓÚ\\s+]{1,25}") || term.matches("[a-zA-ZñÑáéíóúÁÉÍÓÚ+]{1,25}")){
		...
		btnIngresarConfirm.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				try {
					...
					int option = JOptionPane.showConfirmDialog(null, "¿Está seguro de ingresar los registros de un nuevo paciente?", "Confirmar ingreso de datos", JOptionPane.OK_CANCEL_OPTION);
					if(option == 0){
	...
	private int getCboBuscarPor(){
		...
		if(cboSelect.contentEquals("Código de paciente"))
			return 1;
