...
//					txtNumero.setText("");
						...
						if(cboSelect == 1){
							if(term.matches("EMP\\d{3}")){
								...
								if(buscarEmpleado != null){
									...
									mensaje("No hay registros de empleados con este código.");
								}
							}
							else {
		...
		tabla.addColumn("Código de empleado");
		...
		btnIngresarConfirm.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				try {
					...
					int option = JOptionPane.showConfirmDialog(null, "¿Está seguro de ingresar los registros de un nuevo empleado?", "Confirmar ingreso de datos", JOptionPane.OK_CANCEL_OPTION);
					if(option == 0){
		...
		btnModificarConfirm.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				try {
					...
					int option = JOptionPane.showConfirmDialog(null, "¿Está seguro de modificar los registros de este empleado?", "Confirmar modificación", JOptionPane.OK_CANCEL_OPTION);
					if(option == 0){
		...
		btnEliminarConfirm.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if(dropEmpleado != null){
					int option = JOptionPane.showConfirmDialog(null, "¿Está seguro de eliminar los registros de este empleado?", "Confirmar eliminación", JOptionPane.OK_CANCEL_OPTION);
					if(option == 0){
...
//        paciente.cargarPacientes();
	...
	private int getCboBuscarPor(){
		...
		if(cboSelect.contentEquals("Código de empleado"))
			return 1;
	...
	private String validarNombre(){
		String nombre = "";
		if(getNombre().matches("[a-zA-ZñÑáéíóúÁÉÍÓÚ\\s+]{3,25}"))
			nombre = getNombre();
		else {
	...
	private String validarApellidos(){
		String apellidos = "";
		if(getApellidos().matches("[a-zA-ZñÑáéíóúÁÉÍÓÚ\\s+]{3,25}"))
	...
	private int validarTelefono(){
		...
		if(!txtTelefono.getText().contentEquals("") && (txtTelefono.getText().length() == 7 || txtTelefono.getText().length() == 9))
			...
			mensaje("Ingrese correctamente el número de teléfono.\nEjemplo: \"56368957\" ó \"987654321\"\nSólo números, sin espacios ni guiones.");
	...
	private String validarUsuario(){
		String usuario = "";
		if(getNombre().matches("[a-zA-ZñÑáéíóúÁÉÍÓÚ]{3,25}"))
			usuario = getUsuario();
		else {
	...
	private String validarPassword(){
		String password = "";
		if(getPassword().matches("[a-zA-ZñÑáéíóúÁÉÍÓÚ1-9]{3,25}")){
			password = getPassword();
		}
		else {
			mensaje("Ingrese una contraseña correcta por favor.\nEjemplo: \"Gonzalo\".\nSólo letras y números");
